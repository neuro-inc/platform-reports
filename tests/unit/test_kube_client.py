...
class TestPod:
    def test_from_payload(self) -> None:
        result = Pod.from_payload(
            {
                ...
                "spec": {"containers": [{"name": "job", "image": "ubuntu"}]},
        ...
        assert result == Pod(
            ...
            containers=[Container(name="job", image="ubuntu")],
...
class TestContainer:
    def test_from_payload(self) -> None:
        result = Container.from_payload(
            {"name": "job", "image": "ubuntu", "resources": {"requests": {"cpu": "1"}}}
        )

        assert result == Container(
            name="job", image="ubuntu", resource_requests=Resources(cpu_m=1000)
        )

    def test_from_payload_without_resource_requests(self) -> None:
        result = Container.from_payload(
            {"name": "job", "image": "ubuntu", "resources": {"limits": {"cpu": "1"}}}
        )
        assert result == Container(name="job", image="ubuntu")

        result = Container.from_payload({"name": "job", "image": "ubuntu"})
        assert result == Container(name="job", image="ubuntu")
